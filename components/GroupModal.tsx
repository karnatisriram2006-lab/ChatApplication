                    ...
                    <div className="space-y-3">
                        ...
                        <div className="max-h-64 overflow-y-auto pr-2 flex flex-col gap-2 custom-scrollbar">
                            {filteredContacts.length > 0 ? filteredContacts.map(contact => (
                                <div key={contact.uid} onClick={() => toggleMember(contact.uid)}
                                    className={`flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all duration-200 border ${selectedMembers.includes(contact.uid) ? "bg-primary/8 dark:bg-primary/15 border-primary shadow-sm" : "bg-surface-2/20 dark:bg-surface-2/40 border-transparent hover:bg-surface-2 hover:border-border"}`}>
                                    <div className="flex-1 flex items-center gap-4">
                                        <div className="relative">
                                            <div className="w-11 h-11 rounded-xl glass p-[1.5px] shadow-sm">
                                                <Image src={contact.photoURL || "/user-fill.svg"} alt={contact.displayName} width={40} height={40} className="w-full h-full rounded-[10px] object-cover" />
                                            </div>
                                        ...
                                        <p className={`text-[15px] font-bold tracking-tight ${selectedMembers.includes(contact.uid) ? 'text-primary' : 'text-text-primary'}`}>{contact.displayName}</p>
                                    </div>
                                ...
                                <p className="text-center py-8 text-sm text-text-muted font-medium italic opacity-60">No contacts found. Add contacts first.</p>
                            )}
                        </div>
                    </div>

                    <button onClick={handleCreateGroup} disabled={isCreating || !groupName.trim() || selectedMembers.length === 0}
